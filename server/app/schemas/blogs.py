...
def _iso(value: Any) -> Optional[str]:
    """Normalise Firestore timestamp / string to ISO-8601 string."""
    if value is None:
        return None
...
def list_blogs(
    ...
    for doc in docs:
        ...
        # In-memory search filter if search term is provided
        if search:
...
def create_blog(
    ...
    # Resolve or generate unique slug
    base_slug = blog_in.slug or _slugify(blog_in.title)
    slug = base_slug
...
def update_blog(
    ...
    if blog_in.slug is not None:
        updates["slug"] = _slugify(blog_in.slug)
...
def get_comments(blog_id: str) -> CommentTreeResponse:
    ...
    # Attach child replies to their root parent comment
...
def create_comment(
    ...
    # If replying to a comment, ensure root parent is targeted (1-layer flattening)
    if comment_in.parent_id:
        ...
        if grandparent_id:
            # The target was already a reply -> flatten to grandparent (root)
            target_parent_id = grandparent_id
        else:
            # The target was a root comment
            target_parent_id = comment_in.parent_id

        # Increment reply count on the resolved root parent
        try:
    ...
    # Increment blog total comment count
    try:
