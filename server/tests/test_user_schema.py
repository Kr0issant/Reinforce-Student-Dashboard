...
class DiscordVerifyRequestTests(unittest.TestCase):
    def test_valid_discord_snowflake(self):
        valid = DiscordVerifyRequest.model_validate({"discord_id": "1549547403819090011"})
        self.assertEqual(valid.discord_id, "1549547403819090011")

    def test_invalid_discord_snowflake_rejected(self):
        for bad in ("12345", "not_a_number", "1234567890123456789012345", ""):
            with self.subTest(bad=bad):
                with self.assertRaises(ValidationError):
                    DiscordVerifyRequest.model_validate({"discord_id": bad})
